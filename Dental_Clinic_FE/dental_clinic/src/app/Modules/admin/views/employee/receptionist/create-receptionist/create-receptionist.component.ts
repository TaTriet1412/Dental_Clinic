...
@Component({
  ...
  styleUrl: './create-receptionist.component.scss'
})
export class CreateReceptionistComponent {
      ...
      this.authService.createAccount(accountCreateReq).subscribe(
        {
          next: async (res: any) => {
            ...
            setTimeout(() => {
              this.goToReceptionistList(); // Chuyển hướng đến danh sách nhân viên
  ...
  async uploadImage(userId: number, file: File): Promise<void> {
    ...
    goToReceptionistList() {
      this.router.navigate([ROUTES.ADMIN.children.EMPLOYEE.children.RECEPTIONIST.children.LIST.fullPath]);
    }
}
