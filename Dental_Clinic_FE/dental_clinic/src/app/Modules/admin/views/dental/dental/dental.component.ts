...
export class DentalComponent implements OnInit, OnDestroy {
  ...
  readonly sortFields$ = new BehaviorSubject<ISorterValue>({ column: 'dentalDate', state: 'asc' });
  ...
  data$: Observable<DentalResponse[]>;
  ...
  readonly columns: (IColumn | string)[] = [
    ...
    { key: 'cared_actor', label: 'Đối tượng' },
  ...
  constructor(
    ...
    this.data$ = combineLatest([this.apiParams$, this.dentalService.getAllDental(),
  ...
  private formatSortParam(sorter: ISorterValue): string {
    if (!sorter || !sorter.column || !sorter.state) {
      return 'dentalDate,asc'; // Default sort
    }
    return `${sorter.column},${sorter.state}`;
  }

  goToCreateDental() {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.DENTAL.children.CREATE.fullPath]);
  }

  goToDetailDental(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.DENTAL.children.DETAIL.fullPath(id)]);
  }

  goToEditDental(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.DENTAL.children.EDIT.fullPath(id)]);
  }

  toggleAble(dentalId: string) {
    this.dentalService.toggleAble(dentalId).subscribe({
      next: (response) => {
        this.snackbar.notifySuccess(response.message);
        this.data$ = this.data$.pipe(
          map((dentals) => {
            return dentals.map((dental) => {
              if (dental.id === dentalId) {
                dental.able = dental.able; // Cập nhật trạng thái able của người dùng
              }
              return dental;
            });
          })
