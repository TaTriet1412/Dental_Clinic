...
export class CategoryComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.data$ = combineLatest([this.apiParams$, this.dentalService.getAllCategory(),
  ...
  goToCreateCategory() {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.CATEGORY.children.CREATE.fullPath]);
  }

  goToDetailCategory(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.CATEGORY.children.DETAIL.fullPath(id)]);
  }

  goToEditCategory(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.SERVICE.children.CATEGORY.children.EDIT.fullPath(id)]);
  }

  toggleAble(categoryId: string) {
    this.dentalService.toggleAbleCategory(categoryId).subscribe({
      next: (response) => {
        this.snackbar.notifySuccess(response.message);
        this.data$ = this.data$.pipe(
          map((dentals) => {
            return dentals.map((dental) => {
              if (dental.id === categoryId) {
