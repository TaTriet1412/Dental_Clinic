...
export class ConsumableMaterialComponent implements OnInit, OnDestroy {
  ...
  goToCreateConsumableMaterial() {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.CONSUMABLE.children.CREATE.fullPath]);
  }

  goToDetailConsumableMaterial(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.CONSUMABLE.children.DETAIL.fullPath(id)]);
  }

  goToEditConsumableMaterial(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.CONSUMABLE.children.EDIT.fullPath(id)]);
  }

  toggleAble(consumableMaterialId: number) {
    this.materialService.toggleAbleMaterial(consumableMaterialId).subscribe({
      next: (response) => {
        this.snackbar.notifySuccess(response.message);
        this.data$ = this.data$.pipe(
          map((consumableMaterials) => {
            return consumableMaterials.map((consumableMaterial) => {
              if (consumableMaterial.id === consumableMaterialId) {
                consumableMaterial.able = consumableMaterial.able; // Cập nhật trạng thái able của người dùng
              }
