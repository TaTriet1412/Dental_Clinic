...
export class DetailCategoryComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    this.categoryId = Number(this.url.snapshot.paramMap.get('id')!) || -1;

    try {
      this.spinner.show();
      const fetchedCategory = await firstValueFrom(this.materialService.getCategoryById(this.categoryId));
