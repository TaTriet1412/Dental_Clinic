...
export class CreateCategoryComponent {
  ...
  onSubmit(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.materialService.createCategory(categoryCreateReq).subscribe(
        {
          next: async (res: any) => {
            this.spinner.hide(); // Ẩn spinner khi hoàn thành xử lý
            this.snackbar.notifySuccess('Tạo danh mục thành công! - Đang chuyển hướng đến danh sách danh mục vật liệu');
            setTimeout(() => {
  ...
  goToCategoryList() {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.CATEGORY.children.LIST.fullPath]);
  }
}
