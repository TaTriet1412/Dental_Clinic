...
export class IngredientComponent implements OnInit, OnDestroy {
  ...
  readonly sortFields$ = new BehaviorSubject<ISorterValue>({ column: 'ingredientDate', state: 'asc' });
  readonly loading$ = new BehaviorSubject<boolean>(true);
  readonly totalPages$ = new BehaviorSubject<number>(1);
  readonly errorMessage$ = new Subject<string>();
  readonly #destroy$ = new Subject<boolean>();
  readonly selectedIngredient$ = new BehaviorSubject<string>('');
  ...
  constructor(
    ...
    this.data$ = combineLatest([this.apiParams$, this.ingredientService.getAllIngredient(),
    this.selectedIngredient$]).pipe(
      tap(() => {
      ...
      map(([params, response]) => {
        ...
        if (this.selectedIngredient) {
          filteredData = filteredData.filter(item => item.ingredientId === this.selectedIngredient);
        }
  ...
  private formatSortParam(sorter: ISorterValue): string {
    if (!sorter || !sorter.column || !sorter.state) {
      return 'ingredientDate,asc'; // Default sort
    }
    return `${sorter.column},${sorter.state}`;
  }

  goToCreateIngredient() {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.INGREDIENT.children.CREATE.fullPath]);
  }

  goToDetailIngredient(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.INGREDIENT.children.DETAIL.fullPath(id)]);
  }

  goToEditIngredient(id: string) {
    this.router.navigate([ROUTES.ADMIN.children.MATERIAL.children.INGREDIENT.children.EDIT.fullPath(id)]);
  }

  toggleAble(ingredientId: number) {
    this.ingredientService.toggleAbleIngredient(ingredientId).subscribe({
      next: (response) => {
          ...
          map((ingredients) => {
            return ingredients.map((ingredient) => {
              if (ingredient.id === ingredientId) {
                ingredient.able = ingredient.able; // Cập nhật trạng thái able của người dùng
              }
              return ingredient;
            });
          })
  ...
  handleIngredientChange(): void {
    this.selectedIngredient$.next(this.selectedIngredient); // Cập nhật observable
