...
export class DentistComponent implements OnInit, OnDestroy {
  ...
  readonly sortFields$ = new BehaviorSubject<ISorterValue>({ column: 'dentistDate', state: 'asc' });
  ...
  ngOnInit(): void {
    console.log('DentistComponent initialized.');
  }
  ...
  private formatSortParam(sorter: ISorterValue): string {
    if (!sorter || !sorter.column || !sorter.state) {
      return 'dentistDate,asc'; // Default sort
    }
    return `${sorter.column},${sorter.state}`;
  }

  goToCreateDentist() {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.DENTIST.children.CREATE.fullPath]);
  }

  goToDetailDentist(id: string) {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.DENTIST.children.DETAIL.fullPath(id)]);
  }

  goToEditDentist(id: string) {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.DENTIST.children.EDIT.fullPath(id)]);
  }
        ...
        this.data$ = this.data$.pipe(
          map((dentists) => {
            return dentists.map((dentist) => {
              if (dentist.id === userId) {
                dentist.ban = dentist.ban; // Cập nhật trạng thái ban của người dùng
              }
              return dentist;
            });
          })
