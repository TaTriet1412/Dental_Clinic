...
export class AssistantComponent implements OnInit, OnDestroy {
  ...
  readonly sortFields$ = new BehaviorSubject<ISorterValue>({ column: 'assistantDate', state: 'asc' });
  ...
  roleId = 4; // ID của vai trò "Lễ tân"
  ...
  ngOnInit(): void {
    console.log('AssistantComponent initialized.');
  }
  ...
  private formatSortParam(sorter: ISorterValue): string {
    if (!sorter || !sorter.column || !sorter.state) {
      return 'assistantDate,asc'; // Default sort
    }
    return `${sorter.column},${sorter.state}`;
  }

  goToCreateAssistant() {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.ASSISTANT.children.CREATE.fullPath]);
  }

  goToDetailAssistant(id: string) {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.ASSISTANT.children.DETAIL.fullPath(id)]);
  }

  goToEditAssistant(id: string) {
    this.router.navigate([ROUTES.RECEPTIONIST.children.EMPLOYEE.children.ASSISTANT.children.EDIT.fullPath(id)]);
  }
          ...
          map((assistants) => {
            return assistants.map((assistant) => {
              if (assistant.id === userId) {
                assistant.ban = assistant.ban; // Cập nhật trạng thái ban của người dùng
              }
              return assistant;
            });
          })
