...
export class EditAssistantComponent implements OnInit {
  ...
  assistantId: number = -1; // Biến để lưu ID người dùng
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      this.salary = this.assistant.salary;
      this.address = this.assistant.address;
      this.gender = this.assistant.gender;
      this.birthday = this.assistant.birthday ? new Date(this.assistant.birthday) : new Date(); // Chuyển đổi chuỗi thành đối tượng Date
      this.selectedImage = this.changeImageServer(this.assistant.img) || '/assets/images/auth/blank_user.png'; // Đường dẫn ảnh mặc định nếu không có ảnh từ API
    } catch (error: any) {
  ...
  maxDateForDatePicker(maxDate: Date): Date {
    const yesterday = maxDate;
  ...
  onSubmit(form: NgForm) {
    ...
    if (form.valid) {
      ...
      console.log(accountUpdateReq.birthday);
      ...
      if (this.selectedFileImage) {
        this.uploadImage(this.assistantId, this.selectedFileImage);
      }
    }
  }
  ...
  async uploadImage(assistantId: number, file: File): Promise<void> {
    this.authService.uploadImg(assistantId, file).subscribe({
      next: (res: any) => {
      },
      error: (err) => {
