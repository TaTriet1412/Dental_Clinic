...
interface DialogData {
  mode: 'add' | 'edit' | 'view'; // Mode of the dialog (add or edit)
  eventData: {
...
export class EventDialogComponent implements OnInit {
  ...
  constructor(
    ...
    this.dialogTitle = this.mode === 'add' ? 'Thêm Lịch Làm Việc Mới' :
                       (this.mode === 'edit' ? 'Sửa Lịch Làm Việc' : 'Chi Tiết Lịch Làm Việc');

    const initialValues = this.extractInitialValues(this.originalEventData, this.mode);

    // --- Thay đổi ở đây: Không dùng 'disabled' nữa ---
    this.eventForm = this.fb.group({
      id: [initialValues.id],
      title: '',
      startDate: [initialValues.startDate, Validators.required], // Chỉ đặt giá trị
      startTime: [initialValues.startTime, Validators.required], // Chỉ đặt giá trị
      endDate: [initialValues.endDate, Validators.required],     // Chỉ đặt giá trị
      endTime: [initialValues.endTime, Validators.required]       // Chỉ đặt giá trị
    });
    // --- Kết thúc thay đổi ---
  }
  ...
  private extractInitialValues(data: any, mode: 'add' | 'edit' | 'view'): any {
    if (mode === 'add') {
      const startDate = this.formatDateForInput(data?.start); // Ngày bắt đầu
      const startTime = '08:00'; // Thời gian bắt đầu mặc định
      const endDate = this.formatDateForInput(data?.start); // Ngày kết thúc mặc định giống ngày bắt đầu
      const endTime = '17:00'; // Thời gian kết thúc mặc định
  ...
  onSave(): void {
    ...
    if (this.eventForm.valid) {
      // const formValue = this.eventForm.getRawValue(); // Hoặc this.eventForm.value
      ...
      this.dialogRef.close({ action: 'save', event: { ...eventDataToReturn, title: '' } });
    } else {
  ...
  onDelete(): void {
    if (this.mode !== 'edit') {
      console.warn('Cannot delete in add or view mode.');
      return;
    }
