...
export class CreatePrescriptionComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    ...
    this.prescriptionService.createPrescription(prescriptionData).subscribe({
      next: (response: any) => {
        this.spinner.hide();
        this.snackBar.notifySuccess((response.message || 'Thêm toa thuốc thành công!') + " - Đang chuyển hướng đến trang danh sách toa thuốc" );
        setTimeout(() => {
          this.goToPrescriptionList()
        }
        , 3000); // Hide spinner after 1 second
      },
      error: (error: any) => {
        this.spinner.hide();
        this.disabledBtnSubmit = false; // Re-enable button on error
        this.snackBar.notifyError(error?.error?.message || 'Có lỗi xảy ra khi thêm toa thuốc.');
  ...
  goToPrescriptionList() {
    this.router.navigate([ROUTES.DENTIST.children.PRESCRIPTION.children.LIST.fullPath]);
  }
}
